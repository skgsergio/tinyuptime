            ...
            {data.map((row: SummaryData, index: number) => (
              <tr key={index} className="hover:bg-gray-700">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-base font-mono">
                  ...
                  <span className="text-gray-300 text-sm"> ({(row.successful_checks / row.total_checks * 100).toFixed(1)}%)</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-base font-mono">
                  <span className="text-red-400">{row.failing_checks}</span>
                  <span className="text-gray-300 text-sm"> ({(row.failing_checks / row.total_checks * 100).toFixed(1)}%)</span>
                </td>
