    ...
    <div className="mt-2">
      <div className="flex h-4 gap-0.5">
        {recentStatuses.map((status, index) => {
          ...
          if (index < tooltipThreshold)
            tooltipPositionClass = "left-0 -translate-x-0";
          else if (index >= recentStatuses.length - tooltipThreshold)
          ...
          return (
            ...
            >
              ...
              />
              <div
                className={`absolute top-6 ${tooltipPositionClass} whitespace-nowrap px-2 py-1 text-sm text-white bg-gray-950 rounded opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity pointer-events-none z-10 max-w-[calc(100vw-2rem)] min-w-0 overflow-x-hidden`}
              >
